...
func (lc *LaptopClient) CreateLaptop(laptop *pb.Laptop) {
	...
	res, err := lc.service.CreateLaptop(ctx, req)
	if err != nil {
...
func (lc *LaptopClient) SearchLaptop(filter *pb.Filter) {
	log.Print("search filter: ", filter)

	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	req := &pb.SearchLaptopRequest{Laptop: filter}
	stream, err := lc.service.SearchLaptop(ctx, req)
	if err != nil {
...
func (lc *LaptopClient) UploadImage(laptopID string, imagePath string) {
	...
	stream, err := lc.service.UploadImage(ctx)
	if err != nil {
...
func (lc *LaptopClient) RateLaptop(laptopIDs []string, scores []float64) error {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	stream, err := lc.service.RateLaptop(ctx)
	if err != nil {
